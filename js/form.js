...
document.addEventListener('DOMContentLoaded', function() {
    ...
    const buttonIcon = submitButton ? submitButton.querySelector('.button-icon') : null;
    ...
    function showError(message) {
        ...
        errorDiv.style.cssText = 'background: rgba(255, 107, 107, 0.1); border-left: 4px solid var(--color-error); padding: 15px; margin-top: 20px; border-radius: 8px; animation: shake 0.5s ease;';
    ...
    async function sendForm(formElement) {
        try {
            // 1. Собираем данные из формы напрямую в объект
            const formData = {
            ...
            // 2. Отправляем на Formspree (ВАШ НОВЫЙ ENDPOINT URL)
    ...
    form.addEventListener('submit', async function(e) {
        ...
        try {
            ...
            if (error.message.includes('Failed to fetch')) {
                ...
                errorMessage = 'Проблема с настройками домена. Проверьте "Restrict to Domain" в Formspree.';
            }
...
function testForm() {
    const form = document.getElementById('questionForm');
    if (form) {
        form.querySelector('#name').value = 'Тестовый пользователь';
        form.querySelector('#email').value = 'test@example.com';
        form.querySelector('#question').value = 'Это тестовое сообщение для проверки формы ' + new Date().toLocaleTimeString();
        console.log('✅ Тестовые данные заполнены');
        console.log('📧 Тестовый email: test@example.com');
    } else {
